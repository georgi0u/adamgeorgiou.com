...
function onActivate(element: HTMLElement, action: () => void) {
...
function writeContactSection() {
    ...
    document.querySelectorAll<HTMLElement>(".email").forEach((element) => {
...
function limitSubContainers() {
    ...
    document.querySelectorAll<HTMLUListElement>(".sub_container.collapsible > ul").forEach((list) => {
        ...
        onActivate(collapse, () => {
            ...
            const header = Array.from(parent.children)
                .find((child): child is HTMLElement => child instanceof HTMLElement && child.tagName === "H1");
            if (header) {
...
function colorResumeSubcategoryTags() {
    const elements = Array.from(document.querySelectorAll<HTMLElement>("#resume .sub_category_tags > li"));
    if (!elements.length) {
        return;
    }
...
function writeAboutControls() {
    ...
    onActivate(expandButton, () => {
        const moreAbout = document.querySelector<HTMLElement>("#more_about");
        if (moreAbout) {
...
function jumbleTheJunk() {
    const drawer = document.querySelector<HTMLUListElement>("#junk_drawer_container > ul");
    if (!drawer) {
        return;
    }
